...
public class PoolTask implements Runnable {
  ...
  public PoolTask(String nsr_id, int pool_size, PoolEngine poolEngine, ActionMonitor actionMonitor)
  ...
  public void run() {
    ...
    Map<String, Map<String, Set<VNFCInstance>>> reservedInstances =
        poolEngine.getReservedInstances(nsr_id);
    ...
    for (String vnfr_id : reservedInstances.keySet()) {
      ...
      for (String vdu_id : reservedInstances.get(vnfr_id).keySet()) {
        ...
        log.debug(
            "Current pool size of NSR::VNFR::VDU: "
                + nsr_id
                + "::"
                + vnfr_id
                + "::"
        ...
        while (launch > 0) {
          ...
          try {
            log.debug(
                "Launching "
                    + launchThisRound
                    + " VNFCInstances for the pool of NSR::VNFR::VDU: "
                    + nsr_id
                    + "::"
                    + vnfr_id
                    + "::"
            ...
            newReservedInstances =
                poolEngine.allocateNewInstance(nsr_id, vnfr_id, vdu_id, launchThisRound);
